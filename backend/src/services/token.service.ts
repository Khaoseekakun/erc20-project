...
export type PublicToken = { id: number; address: string; name: string; symbol: string; decimals: number; balance: string };
...
export async function listWalletTokens(walletId: number): Promise<PublicToken[]> {
  ...
  if (!wallet) throw Object.assign(new Error("ไม่พบ Wallet"), { status: 404 });
...
export async function addTokenToWallet(userId: number, walletId: number, address: string) {
  ...
  if (!isAddress(address)) throw Object.assign(new Error("Contract Address ไม่ถูกต้อง"), { status: 400 });
  const contractAddress = getAddress(address);
  if ((await provider.getCode(contractAddress)) === "0x") throw Object.assign(new Error("ไม่พบ Smart Contract นี้บน Sepolia"), { status: 400 });
  try {
