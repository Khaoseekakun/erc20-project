...
export async function listTransactions(req: Request, res: Response) {
  ...
  if (!Number.isInteger(page) || page < 1 || !Number.isInteger(limit) || limit < 1 || limit > 50) return res.status(400).json({ success: false, message: "พารามิเตอร์ pagination ไม่ถูกต้อง" });
  return res.json({ success: true, ...(await transactionHistory(req.userId!, page, limit)) });
}

export function streamTransactions(req: Request, res: Response) {
