...
export async function verifyBlockchainConfiguration() { const network = await provider.getNetwork(); if (network.chainId !== 11155111n) throw new Error(`Configured RPC is chain ${network.chainId}, not Sepolia (11155111).`); if ((await provider.getCode(env.contractAddress)) === "0x") throw new Error("Contract not found at ERC20_CONTRACT_ADDRESS on Sepolia."); }
