...
const required = ["DATABASE_URL", "JWT_SECRET", "WALLET_ENCRYPTION_KEY", "SEPOLIA_RPC_URL", "ERC20_CONTRACT_ADDRESS", "ADMIN_EMAIL", "DEPLOYER_PRIVATE_KEY"];
for (const key of required)
    if (!process.env[key])
        throw new Error(`Missing required environment variable: ${key}`);
if ((process.env.JWT_SECRET?.length ?? 0) < 32)
    throw new Error("JWT_SECRET must contain at least 32 characters.");
if (!/^[a-fA-F0-9]{64}$/.test(process.env.WALLET_ENCRYPTION_KEY))
    throw new Error("WALLET_ENCRYPTION_KEY must be exactly 64 hexadecimal characters.");
...
if (!isAddress(process.env.ERC20_CONTRACT_ADDRESS))
    throw new Error("ERC20_CONTRACT_ADDRESS is not a valid Ethereum address.");
