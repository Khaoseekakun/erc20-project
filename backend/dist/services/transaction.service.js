...
export async function sendToken(userId, input) {
    if (!isAddress(input.to) || getAddress(input.to) === ZeroAddress)
        throw Object.assign(new Error("Wallet Address ไม่ถูกต้อง"), { status: 400 });
    if (!isAddress(input.tokenAddress))
        throw Object.assign(new Error("Token Contract Address ไม่ถูกต้อง"), { status: 400 });
    ...
    if (!walletToken)
        throw Object.assign(new Error("Token นี้ยังไม่ได้เพิ่มใน Wallet ที่เลือก"), { status: 400 });
    let units;
    try {
        units = parseUnits(input.amount, walletToken.token.decimals);
    }
    catch {
        throw Object.assign(new Error(`จำนวนเงินรองรับทศนิยมได้สูงสุด ${walletToken.token.decimals} ตำแหน่ง`), { status: 400 });
    }
    if (units <= 0n)
        throw Object.assign(new Error("จำนวน Token ต้องมากกว่า 0"), { status: 400 });
    ...
    if (tokenBalance < units)
        throw Object.assign(new Error("ยอด Token ไม่เพียงพอ"), { status: 400 });
    if (ethBalance === 0n)
        throw Object.assign(new Error("Wallet ไม่มี Sepolia ETH สำหรับค่าธรรมเนียมเครือข่าย"), { status: 400 });
...
async function broadcastTransfer(userId, transactionId, wallet, token, to, units) {
    try {
        const signer = new Wallet(decryptPrivateKey(wallet), provider);
        if (signer.address !== wallet.address)
            throw new Error("Wallet signing identity does not match stored address.");
...
function toTransactionResponse(record) {
...
export async function transactionHistory(userId, page, limit) {
